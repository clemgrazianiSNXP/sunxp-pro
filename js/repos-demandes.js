  ...
  setTimeout(() => {
    ...
    wrap.querySelectorAll('.repos-refuse').forEach(btn => {
      btn.onclick = () => {
        ...
        const id = btn.dataset.id;
        const d = demandes.find(x => x.id === id);
        if (d) { d.statut = 'refusee'; d.reponse = motif ? 'Refusée : ' + motif : 'Demande refusée.'; saveReposDemandes(stationId, demandes); renderReposModule(); if (typeof sendPushToStation === 'function') sendPushToStation(stationId, '❌ Repos refusé', 'Ton repos du ' + new Date(d.date1).toLocaleDateString('fr-FR') + ' est refusé.' + (motif ? ' Motif : ' + motif : ''), d.chauffeurId); }
      };
    });
  }, 0);
