...
(function initTopbar() {
  ...
  logo.alt = 'SunXP';
  logo.style.cssText = 'height:26px;width:auto;opacity:0.8;position:absolute;left:50%;transform:translateX(-50%);';
  ...
  themeBtn.onclick = () => {
    ...
    // Re-render heures pour mettre à jour les couleurs de vagues (seulement si on est sur l'onglet heures)
    if (typeof renderHeures === 'function') {
      const heuresModule = document.getElementById('module-heures');
      if (heuresModule && heuresModule.classList.contains('active')) {
  ...
  logoutBtn.style.cssText = 'background:transparent;border:1px solid var(--border);border-radius:5px;padding:2px 8px;font-size:11px;cursor:pointer;color:var(--text-muted);transition:border-color 0.18s;';
