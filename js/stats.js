...
async function getWeeksListAsync(type) {
  const prefix = getStationId() + '-stats-' + type + '-';
  // D'abord chercher dans localStorage
  const localWeeks = [];
  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i);
    if (k && k.startsWith(prefix)) localWeeks.push(k.replace(prefix, ''));
  }
  if (localWeeks.length) return localWeeks.sort().reverse();

  // Si localStorage vide, charger depuis Supabase
  if (typeof sb === 'function' && sb()) {
    try {
      const { data } = await sb().from('stats')
        .select('semaine, data')
        .eq('station_id', getStationId())
        .eq('type', type)
        .order('semaine', { ascending: false });
      if (data && data.length) {
        // Remettre dans localStorage
        data.forEach(row => {
          if (row.semaine && row.data) {
            localStorage.setItem(prefix + row.semaine, JSON.stringify(row.data));
          }
        });
        return data.map(r => r.semaine).sort().reverse();
      }
    } catch(e) { console.warn('getWeeksListAsync error:', e.message); }
  }
  return [];
}
